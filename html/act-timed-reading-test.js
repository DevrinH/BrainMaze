document.addEventListener("DOMContentLoaded", () => {
    ...
    function startReadingSection() {
        ...
        setTimeout(endReadingSection, 35 * 60 * 1000); // End after 35 minutes
    ...
    function showFinalScore() {
        ...
        let maxPossibleScore = (13 * 1) + (14 * 2) + (13 * 3); // Assume 40 questions (standard ACT Reading)
        let rawScore = score;
        readingScore = Math.round((rawScore / maxPossibleScore) * 35 + 1);

        // Store score in localStorage
        localStorage.setItem("actReadingScore", readingScore);

        // Do not write to actScoreHistory to avoid affecting the chart
        console.log(`Timed Reading score ${readingScore} saved to actReadingScore for ${new Date().toLocaleDateString("en-CA")}`);

        // Save test completion metadata
        saveTestCompletion("ACT-Reading");

        // Update UI
        document.getElementById("question-container").classList.remove("hide");
        passageElement.innerHTML = "";
        questionElement.innerHTML = `<p><strong>ACT Reading Score:</strong> ${readingScore} / 36</p>`;
