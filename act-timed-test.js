document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!actIntroContainer || !startTestButton || !questionElement || !passageElement || !answerButtons || !nextButton || !countdownEl || !continueButton) {
    ...
    // Placeholder question arrays (replace with actual questions)
    const mathQuestions = [
        {
    ...
    const readingQuestions = [
        {
            passage: "The forest was silent, save for the rustle of leaves. Clara paused, her breath visible in the chilly air. She’d come seeking solitude, but now felt an eerie sense of being watched.",
            question: "What does the passage suggest about Clara’s experience?",
            answers: [
                { text: "A) She feels uneasy despite seeking solitude.", correct: true },
                { text: "B) She is excited by the forest’s beauty.", correct: false },
                { text: "C) She is lost and confused.", correct: false },
                { text: "D) She feels refreshed and calm.", correct: false },
            ],
            type: "reading",
            difficulty: "medium",
            category: "inference"
    ...
    const scienceQuestions = [
        {
            passage: "The experiment showed a sudden spike in temperature at 10 minutes. Researchers noted inconsistent pressure readings, suggesting a possible equipment malfunction.",
            question: "What might the researchers infer from the data?",
            answers: [
                { text: "A) The equipment may be faulty.", correct: true },
                { text: "B) The reaction was successful.", correct: false },
                { text: "C) Temperature had no effect.", correct: false },
                { text: "D) Pressure was stable.", correct: false },
            ],
            type: "science",
            difficulty: "medium",
    ...
    function showScore() {
        ...
        if (currentSection === "english") {
            maxPossibleScore = (25 * 1) + (25 * 2) + (25 * 3);
        } else if (currentSection === "math") {
            maxPossibleScore = (20 * 1) + (20 * 2) + (20 * 3);
        } else {
            maxPossibleScore = (13 * 1) + (14 * 2) + (13 * 3);
        }
        let rawScore = score;
    ...
    function generateExplanation(response) {
        ...
        } else if (passageText.includes("The forest was silent")) {
            return "Clara sought solitude but feels uneasy due to the 'eerie sense of being watched,' supporting A). The passage doesn’t suggest excitement (B), confusion (C), or calmness (D).";
        } else if (passageText.includes("The experiment showed a sudden spike")) {
            return "Inconsistent pressure readings and a temperature spike suggest possible equipment malfunction, making A) correct. The passage doesn’t indicate a successful reaction (B), no effect (C), or stable pressure (D).";
        } else if (response.question.includes("If x + 5 = 12")) {
            return "Solving x + 5 = 12 gives x = 12 - 5 = 7, so B) is correct.";
        }
